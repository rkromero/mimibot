/**
 * Schedulers internos del server (solo runtime Node; lo carga
 * instrumentation.ts al arrancar).
 *
 ...
 *
 * 2. Programador de seguimientos: cada 5 minutos procesa los seguimientos
 *    vencidos (POST /api/followup/process hace lo mismo a mano o desde un
 *    cron externo). Sin esto, en Railway no hay nada que dispare los
 *    seguimientos automáticos (propuesta enviada, estancamiento, sin
...
export function registrarSchedulers(): void {
  ...
  if (globalThis.__followupScheduler) return
  ...
  globalThis.__followupScheduler = setInterval(() => void tick(), INTERVALO_FOLLOWUP_MS)
}
