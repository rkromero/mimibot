...
export const { handlers, signIn, signOut, auth } = NextAuth({
  ...
  callbacks: {
    ...
    async session({ session, token }) {
      let dbUser: { role: 'admin' | 'gerente' | 'agent' | 'vendedor' | 'fabrica'; isActive: boolean; avatarColor: string } | undefined
      try {
      ...
      return {
        ...
        user: {
          ...session.user,
          id: token.sub!,
          role: dbUser?.role ?? ((token.role as 'admin' | 'gerente' | 'agent' | 'vendedor' | 'fabrica' | undefined) ?? 'agent'),
