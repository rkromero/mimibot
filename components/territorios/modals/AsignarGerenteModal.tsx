...
export default function AsignarGerenteModal({ territorioId, onClose, onDone }: Props) {
  ...
  const { data: usuarios = [] } = useQuery<User[]>({
    ...
    queryFn: async () => {
      const res = await fetch('/api/users?role=gerente')
      if (!res.ok) return []
  ...
  const handleConfirm = async () => {
    ...
    setError(null)
    try {
      const res = await fetch(`/api/territorios/${territorioId}/gerentes`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ gerenteId: selected }),
      })
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
      ...
      <div className="relative bg-card border border-border rounded-lg shadow-xl w-full max-w-md mx-4 p-5 space-y-4">
        <div className="flex items-center justify-between">
          <h3 className="text-sm font-semibold text-foreground">Agregar gerente</h3>
        ...
        {usuarios.length === 0 && (
          <p className="text-xs text-muted-foreground text-center py-2">
            No hay usuarios con rol gerente. Primero asigná el rol desde Configuración &gt; Equipo.
          </p>
        )}
