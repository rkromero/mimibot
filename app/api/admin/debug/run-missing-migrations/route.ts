...
// Admin-only POST endpoint that idempotently applies migrations 0008 and 0009
// to the deployed database. These migrations were declared in the Drizzle
// journal but never actually ran in production (only 6 of 12 are present in
// __drizzle_migrations), which left the `productos` table missing 8 columns
// and the `stock_movements` table missing entirely.
//
...
const MIGRATION_0008_STATEMENTS: string[] = [
  // 1. Create unidad_venta enum (skip if already exists)
  `DO $$ BEGIN
    CREATE TYPE "unidad_venta" AS ENUM('unidad', 'caja_12', 'caja_24', 'display');
  EXCEPTION WHEN duplicate_object THEN NULL;
  END $$`,

  // 2. Add new columns to productos
  ...
  // 3. Auto-generate SKUs for existing products that don't have one
  `DO $$
  DECLARE
    r RECORD;
    counter INTEGER := 1;
...
const MIGRATION_0009_STATEMENTS: string[] = [
  ...
  // 4. Alert configuration fields on business_config
...
export async function POST(_req: NextRequest) {
  try {
    ...
    return NextResponse.json({
      ...
      results,
      productosColumns: Array.isArray(cols) ? cols : (cols as { rows?: unknown[] }).rows ?? [],
      stockMovementsTable: Array.isArray(stockExists) ? stockExists : (stockExists as { rows?: unknown[] }).rows ?? [],
    })
  } catch (err) {
...
export async function GET(_req: NextRequest) {
  return NextResponse.json(
    { error: 'Use POST. This endpoint applies missing migrations 0008 and 0009.' },
    { status: 405 },
  )
}
